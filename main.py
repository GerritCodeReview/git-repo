                ...
                if isinstance(e, RepoExitError):
                    ...
                    for error in aggregated_errors:
                        ...
                        git_trace2_event_log.ErrorEvent(
                            f"""AggregateExitError:{type(error).__name__}
                            Project:{project}
                            Message:{error}"""
                        )
                if not ok:
...
def _Main(argv):
    ...
    try:
        ...
        exception_name = type(e).__name__
        aggregate_errors = e.aggregate_errors or []
        shortened_aggregate_errors = aggregate_errors[:5]
        result = e.exit_code
        print("fatal: %s" % e, file=sys.stderr)
        if shortened_aggregate_errors:
            print(f"{exception_name} Aggregate Errors")
            for err in shortened_aggregate_errors:
                print(err)
        if len(shortened_aggregate_errors) < len(aggregate_errors):
            diff = len(aggregate_errors) - len(shortened_aggregate_errors)
