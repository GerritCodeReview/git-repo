...
class _Repo(object):
  ...
  def _Run(self, name, gopts, argv):
    ...
    try:
      ...
      this_manifest_only = getattr(copts, 'this_manifest_only', None)
      parent_manifest = (not getattr(copts, 'no_parent_manifest', False) and
                     not cmd.MULTI_MANIFEST_SUPPORT)
      need_outer = not this_manifest_only and parent_manifest
      if need_outer and repo_client.manifest.is_submanifest:
        # This is a submanifest, and the command does not support multi-manifest.
        # Run the command in the topmost parent, which will (eventually) run the
        # command in this tree, with --no-parent-manifest.
        gopts.submanifest_path = ''
        result = self._Run(name, gopts, argv)
      elif cmd.MULTI_MANIFEST_SUPPORT or this_manifest_only:
        result = cmd.Execute(copts, cargs)
      else:
        # No multi-manifest support. Run the command in the current (sub)manifest,
        # and then any child submanifests.
        result = cmd.Execute(copts, cargs)
        for submanifest in repo_client.manifest.submanifests.values():
          ...
          child_argv = argv[:]
          child_argv.append('--no-parent-manifest')
          if hasattr(copts, 'manifest_url'):
