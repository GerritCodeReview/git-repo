...
class _Repo(object):
  ...
  def _RunInnertrees(self, cmd, gopts, argv, copts, cargs):
    ...
    for name in m.innertrees:
      ...
      if tree.sharing == 'import':
        # The build orchestrator imports the tree.
        print('[%s] Not importing shared tree' % spec.name, file=sys.stderr)
        continue
      ...
      if os.path.exists(repodir):
        ...
        if hasattr(copts, 'innertree'):
          for arg in copts.innertree or []:
            inner_name, inner_grouplist = arg.split(':', 1)
            if name == inner_name:
              # This will overwrite any previous `--groups` argument.
              inner_args.extend(['--groups', inner_grouplist])
