...
class _Repo(object):
  ...
  def _Run(self, name, gopts, argv):
    ...
    try:
      ...
      # Subcommands opt-in to multi-tree support.
      multi_tree_support = getattr(cmd, 'multi_tree_support', None)

      # If --this-tree-only was not on the command line, allow the subcommand to
      # specify a default.
      this_tree_only = getattr(copts, 'this_tree_only',
                               getattr(cmd, 'this_tree_only', False))
      # Should we re-run in the parent tree?
      parent_tree = (not getattr(copts, 'no_parent_tree', False) and
                     not multi_tree_support)

      is_innertree = os.path.exists(os.path.join(self.repodir, HAS_PARENT_FILE_NAME))
      if multi_tree_support:
        ...
        result = cmd.Execute(copts, cargs)
      elif parent_tree and is_innertree:
        # We are an innertree, and the command does not support multi-tree.
        # Re-run the command in our parent directory, which will (eventually)
        # run the command in this tree, with --no-parent-tree.
        result = subprocess.run(
            self._RunGatherArgs(cmd, gopts, argv),
            cwd=os.path.join(cmd.manifest.topdir, '..'), check=True).returncode
      else:
        # We are not in an innertree, and the command does not support
        # multi-tree.  Also run the command in the innertrees.
        # NOTE: `init` currently relies on this path to bootstrap the innertrees.
        result = cmd.Execute(copts, cargs)
