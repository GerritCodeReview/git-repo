  ...
  def _RunInnertrees(self, cmd, gopts, argv, copts, cargs):
    ...
    for name in m.innertrees:
      ...
      if os.path.exists(repodir):
        ...
        inner_args=['python3', os.path.join(m.repodir, 'repo', 'repo')]
        for k, v in gopts.__dict__.items():
          if v is not None and k in ('event_log', 'git_trace2_event_log'):
            inner_args.extend('--' + k.replace('_', '-'), v)
          elif v is True:
            inner_args.append('--' + k.replace('_', '-'))
        inner_args.append(cmd.NAME)
        inner_args.extend(argv)
