...
def _SetTraceFileObj():
  global _TRACE_FILE_OBJ
  trace_file = _GetTraceFile()
  ClearOldTraces(trace_file)
  _TRACE_FILE_OBJ = open(trace_file, 'w')
...
class Trace(ContextDecorator):
    ...
    def __enter__(self):
      ...
      print(print_msg, file=_TRACE_FILE_OBJ)
      if _TRACE_TO_STDERR:
...
def ClearOldTraces(trace_file):
    if os.path.isfile(trace_file) and os.path.getsize(trace_file)/(1024*1024) > _MAX_SIZE:
      temp_file = trace_file + 'tmp'
      with open(trace_file, 'r', errors='ignore') as fin:
