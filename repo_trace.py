...
_MAX_SIZE = .0005#70  # in MiB
_NEW_COMMAND_SEP = '+++++++++++++++NEW COMMAND+++++++++++++++++++'


def IsTraceToStderr():
  """Whether or not traces are written to stderr."""
...
def SetTraceToStderr():
  """Set the _TRACE_TO_STDERR variable."""
  global _TRACE_TO_STDERR
  _TRACE_TO_STDERR = True


def SetTrace():
  """Set the _TRACE variable."""
  global _TRACE
  _TRACE = True


def _SetTraceFile(quiet):
  """Set the _TRACE_FILE variable."""
  global _TRACE_FILE
...
def _ClearOldTraces():
  ...
  try:
    with open(_TRACE_FILE, 'r', errors='ignore') as f:
      if os.stat(f.name).st_size / (1024 * 1024) <= _MAX_SIZE:
        return
