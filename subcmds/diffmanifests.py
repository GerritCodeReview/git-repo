...
"""
  ...
  def _Options(self, p):
    ...
    p.add_option('--pretty-format',
                 ...
                 metavar='<string>',
                 help='display log entries using a custom pretty format string')
  ...
  def _printLogs(self, project, otherProject, raw=False, color=True, pretty_format=None):
    ...
    logs = project.getAddedAndRemovedLogs(otherProject, oneline=(pretty_format is None),
                                          color=color, pretty_format=pretty_format)
    if logs['removed']:
