...
class Diffmanifests(PagedCommand):
  ...
  diff would only show a diff of sha1s for exemple, this command will display
...
project pattern will be <status> <name> <revision from> [<revision to>] and the
...
for a commit.
Only changed projects may contains commits, and commit status always start with
...
"""
  ...
  def Execute(self, opt, args):
    ...
    self.printText = self.out.printer('text')
    if opt.color:
      self.printProject = self.out.printer('project', attr = 'bold')
      self.printAdded = self.out.printer('green', fg = 'green', attr = 'bold')
      self.printRemoved = self.out.printer('red', fg = 'red', attr = 'bold')
      self.printRevision = self.out.printer('revision', fg = 'yellow')
    else:
