...
"""
  ...
  def SetVariablesFromOptions(self, opt):
    ...
    For GitcInit, this will set the repodir to be in the local
    ...
    # Parse the manifest_url so that when we can generate remote urls for each
    # project.
    parsed_manifest_url = urlparse.urlparse(opt.manifest_url)
    self.remote_baseurl = '://'.join([parsed_manifest_url.scheme,
                                      parsed_manifest_url.netloc])
  ...
  def Execute(self, opt, args):
    ...
    if opt.manifest_file:
      ...
      self._GenerateGITCManifest(opt.manifest_branch)
  ...
  def _SetProjectRevisions(self, projects, branch):
    ...
    project_gitcmds = [(
        project, git_command.GitCommand(None,
                                        ['ls-remote',
                                         os.path.join(self.remote_baseurl,
                                         project.name),
  ...
  def _GenerateGITCManifest(self, branch):
