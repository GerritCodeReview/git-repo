...
"""
  ...
  def _SyncManifest(self, opt):
    ...
    if not is_new:
      ...
      if bool(opt.standalone_manifest) or (
        was_standalone_manifest and bool(opt.manifest_url)):
        m.config.ClearCache()
    ...
    if bool(opt.standalone_manifest):
      ...
      # init.
      standalone_manifest = m.config.GetBoolean('manifest.standalone')
    m.config.SetBoolean('manifest.standalone', standalone_manifest)
    ...
    if standalone_manifest:
      if is_new:
        ...
        manifest_name = os.path.basename(opt.manifest_url)
        m.config.SetString('manifest.name', manifest_name)
      opt.manifest_name = m.config.GetString('manifest.name')
      return
