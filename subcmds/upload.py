...
"""
  ...
  def _SingleBranch(self, opt, branch, people):
    ...
    if answer is None:
      ...
      print('Upload project %s/ to remote branch %s%s:' %
            (project.RelPath(opt.this_tree_only), destination,
  ...
  def _MultipleBranches(self, opt, pending, people):
    ...
    for project, avail in pending:
      script.append('#')
      script.append('# project %s/:' % project.RelPath(opt.this_tree_only))
      ...
      projects[project.RelPath(opt.this_tree_only)] = project
    ...
    for line in script:
      ...
      if m:
        ...
        if not branch:
          _die('branch %s not in %s', name, project.RelPath(opt.this_tree_only))
        todo.append(branch)
    if not todo:
      ...
      for branch in todo:
        if not branch.uploaded:
          ...
          print(('[FAILED] %-15s %-15s' + fmt) % (
              branch.project.RelPath(opt.this_tree_only) + '/',
              branch.name,
              str(branch.error)),
              file=sys.stderr)
      print()
  ...
  def Execute(self, opt, args):
    projects = self.GetProjects(args, all_manifests=not opt.this_tree_only)
      ...
      for result in results:
        ...
        if avail is None:
          print('repo: error: %s: Unable to upload branch "%s". '
                ...
                (project.RelPath(opt.this_tree_only), project.CurrentBranch,
