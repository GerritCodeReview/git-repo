...
def _ConfirmManyUploads(multiple_branches=False):
  if multiple_branches:
    print("ATTENTION: One or more branches has an unusually high number of "
          "commits.")
  else:
    print("ATTENTION: You are uploading an unusually high number of commits.")
  print("YOU PROBABLY DO NOT MEAN TO DO THIS. "
        "(Did you rebase across branches?)")
...
"""
      ...
      for branch in todo:
        if not branch.uploaded:
          ...
          print(('[FAILED] %-15s %-15s' + fmt) % (
                 ...
                 branch.name, \
                 str(branch.error)), file=sys.stderr)
      print(file=sys.stderr)
  ...
  def Execute(self, opt, args):
    ...
    if not pending:
      print("no branches ready for upload")
