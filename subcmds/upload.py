...
"""
  ...
  def _Options(self, p):
    ...
    p.add_option('--autosquash',
                 ...
                 help='squash commits first so each branch is its own CL. Unsquashes after upload')
    ...
    try:
      if (opt.autosquash):
        if (len(projects) == 1):
          ...
          print("WARNING: Autosquash aborted. Autosquash doesn't work on multiple projects."
                "Did you make sure to specify only the current directory?")
          return 1
