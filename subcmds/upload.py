...
The --commit-fixups option allows you to automatically handle fixups
generated by pre-upload hooks. If a hook provides a fixup command,
repo will run it, stage the modified files, and create a commit.
For the HEAD commit, it will amend it. For older commits in a chain,
it will create a 'fixup!' commit that can be consumed by
