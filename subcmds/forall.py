...
"""
  ...
  def Execute(self, opt, args):
    ...
    for (cnt, p) in enumerate(projects):
      args_list.append(
        [self.SerialiseProject(p), mirror, opt, cmd, shell, cnt, cnt == 0,
         self.manifest.manifestProject.config])

    results = [
      pool.apply_async(DoWorkWrapper, args=args)
    ...
    try:
      ...
      print('Got an exception, terminating the pool: %r' % e,
            file=sys.stderr)
      pool.terminate()
    finally:
      pool.join()
...
def DoWorkWrapper(project, *args):
...
def DoWork(project, mirror, opt, cmd, shell, cnt, first, config):
  env = os.environ.copy()
  def setenv(name, val):
    if val is None:
      val = ''
