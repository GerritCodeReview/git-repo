...
"""
  ...
  def Execute(self, opt, args):
    ...
    if self.jobs == 1:
      for project in self.GetProjects(args):
        if (self.SubExecute(project, mirror, opt, cmd, shell, first, out) != 0):
  ...
  def SubExecuteWrapper(self, sem, project, mirror, opt, cmd, shell, first, out):
      if (self.SubExecute(project, mirror, opt, cmd, shell, first, out) != 0):
        if opt.abort_on_errors:
          print("error: %s: Aborting due to previous error" % project.relpath,
                file=sys.stderr)
  ...
  def SubExecute(self, project, mirror, opt, cmd, shell, first, out):
