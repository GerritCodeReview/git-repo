...
"""
  ...
  def _GetRemoteHash(self, projects):
    import math
    out = {}
    max_connection = self.opt.max_connection
    ...
    for execute_step in xrange(execute_steps):
      ...
      for key in output_dict:
        if not output_dict[key].Wait() == 0:
          raise GitParallelError(output_dict[key].stderr.strip())
  ...
  def _Output(self, opt):
    ...
    if opt.peg_remote_rev:
      projects = self._GetRemoteProjects()
      hash_dict = self._GetRemoteHash(projects)
