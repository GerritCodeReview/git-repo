...
"""
    ...
    def _Options(self, p):
        p.add_option(
            ...
            "--force",
            action="store_true",
            help="force wipe in the case of shared projects or uncommitted changes",
        )
    ...
    def Execute(self, opt, args: List[str]):
        ...
        for project in projects_to_wipe:
            # Safety check: Do not allow wiping the manifest project.
            ...
            try:
                ...
                # The repo is corrupted, incomplete, or the git binary is unhappy.
                # We treat this as "uncommitted/dirty" to force the user to acknowledge
                # they are wiping a broken state by using --force.
                uncommitted_projects.append(f"{project.name} (broken/incomplete)")
        ...
        if (uncommitted_projects or shared_objdirs) and not opt.force:
            ...
            if uncommitted_projects:
                error_messages.append(
                    "The following projects have uncommitted changes or are corrupted:\n"
  ...
  Projects not to be wiped: {', '.join(sorted(list(other_users)))}"""
        ...
        for project in projects_to_wipe:
            try:
                # We force the delete here because we have already performed our
                # own safety checks (dirty/shared) above.
                project.DeleteWorktree(force=True)
                successful_wipes.add(project.relpath)
            except DeleteWorktreeError as e:
                print(f"error: failed to wipe {project.name}: {e}", file=sys.stderr)
        ...
        for objdir in objdirs_to_delete:
            ...
            # Check if every project that uses this objdir has been successfully processed.
            # If a project failed to wipe (e.g. permission error), we must NOT delete
            # the object directory, or we will corrupt that remaining project.
