...
"""
  ...
  def repack_projects(self, projects: list[Project], opt):
    ...
    for project in projects:
      ...
      if project.clone_depth and project.manifest.CloneFilterForDepth == "blob:none":
    ...
    for project in repack_projects:
      ...
      # Prepare workspace for repacking - remove all unreachable refs and their
      # objects.
      GitCommand(
          project,
      ...
      remote_objects_cmd = GitCommand(
          project,
          ["rev-list", "--objects", f"--remotes={project.remote.name}",
           "--filter=blob:none"],
          capture_stdout=True,
          verify_command=True,
      )
