...
"""
    ...
    def _SmartSyncSetup(self, opt, smart_sync_manifest_path, manifest):
        ...
        if shutil.which(helper_binary):
            ...
            proxy_url = msg
            if not proxy_url.startswith("http://") and not proxy_url.startswith(
                "https://"
            ):
            ...
            if scheme not in ("http", "https"):
                clean_manifest_server = manifest_server.replace(
                    f"{scheme}://", "http://", 1
                )

            try:
                server = xmlrpc.client.Server(
                    clean_manifest_server, transport=transport
                )
                if opt.smart_sync:
                ...
                if success:
                    ...
                    raise SmartSyncError(
                        "error: manifest server RPC call failed: %s"
                        % manifest_str
                    )
            except Exception as e:
                p.kill()
        ...
        if scheme not in (
            ...
            raise SmartSyncError(
                ...
                "If this is a custom scheme, please ensure the local "
                f"helper '{helper_binary}' is installed in your PATH."
            )
