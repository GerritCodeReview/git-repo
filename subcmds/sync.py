...
"""
  ...
  def _CheckoutOne(self, opt, project, lock, pm, err_event, failing_repos_list):
    ...
    Args:
      ...
      err_event: We'll set this event in the case of an error (after printing
          out info about the error).
      failing_repos_list: a list of strings, paths to git repos where checkout
          failed.
    Returns:
      Whether the fetch was successful.
    ...
    try:
      ...
      if did_lock:
        if not success:
          failing_repos_list.append(project.relpath)
        lock.release()
      finish = time.time()
    ...
    for project in all_projects:
      ...
      if project.worktree:
        kwargs = dict(opt=opt,
                      ...
                      pm=pm,
                      err_event=err_event,
                      failing_repos_list=failing_repos)
        if syncjobs > 1:
