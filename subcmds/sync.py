...
"""
  ...
  def _Options(self, p, show_smart=True):
    ...
    p.add_option('--no-post-sync-hook',
                 dest='bypass_post_sync_hook', action='store_true',
                 help='Do not run the post-sync hook.')
    if show_smart:
  ...
  def Execute(self, opt, args):
    ...
    ret = syncbuf.Finish()
    # if repohook repository does not exist, no need to run hook    
    if (self.manifest.repo_hooks_project and
        os.path.exists(self.manifest.repo_hooks_project.gitdir) and
        not opt.bypass_post_sync_hook):
