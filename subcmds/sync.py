...
"""
  ...
  def _FetchProject(self, opt, project, err_event):
    ...
    while True:
      ...
      if cnt < opt.retry_count:
        cnt += 1
        print ('info: Retrying to sync %s (%d/%d retries)' % \
            (project.name, cnt, opt.retry_count), file=sys.stderr)
        if opt.retry_timeout > 0:
          print ('info: Waiting %s seconds before retrying' % opt.retry_timeout, file=sys.stderr)
  ...
  def _FetchHelper(self, opt, project, lock, fetched, pm, sem, err_event):
    ...
    try:
      try:
        success = self._FetchProject(opt, project, err_event)
