...
from git_command import GIT, ssh_sock
...
uncommitted changes are present' % project.relpath
  ...
  def Execute(self, opt, args):
    ...
    if not opt.local_only:
      ...
      filtered = _ProjectsNeedingFetch(all, current_sha, mp)
...
def _FindProjectsRemoteHEAD(manifest):
    ...
    control_master = ""
    if ssh_proxy:
      control_master = "-o ControlPath %s" % ssh_sock()
...
def _ProjectsNeedingFetch(projects, shas, manifest):
  result = []
  b = manifest.GetBranch(manifest.CurrentBranch)
  branch = b.merge

  for project in projects:
    # skip syncing the project if it is not in the same branch as the
    # default one in the manifest.
    if project.name in shas and project.revisionExpr == branch:
      try:
