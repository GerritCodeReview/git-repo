...
"""
  ...
  def Execute(self, opt, args):
    ...
    sync_state = SyncState(config=mp.config, options=opt, sync_time=latest_sync_time)
    mp.config.SetSyncState(sync_state)
    self.git_event_log.AddSyncStateEvents(mp.config.DumpConfigDict(), 'latest_sync_state')
