...
"""
    ...
    def _Options(self, p, show_smart=True):
        ...
        p.add_option(
            ...
            dest="force_rebase",
            action="store_true",
            help="Rebase local commits regardless of whether they are published or not"
        )
        p.add_option(
            "-l",
            "--local-only",
    ...
    def _CheckoutOne(self, detach_head, force_sync, force_rebase, project):
        ...
        success = False
        try:
            project.Sync_LocalHalf(syncbuf, force_sync=force_sync, force_rebase=force_rebase)
    ...
    def _Checkout(self, all_projects, opt, err_results):
        ...
        return (
            self.ExecuteInParallel(
                opt.jobs_checkout,
                functools.partial(
                    self._CheckoutOne, opt.detach_head, opt.force_sync, opt.force_rebase
                ),
                all_projects,
