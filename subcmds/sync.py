...
from error import RepoChangedException, GitError, RepoError, RepoExitError
...
"""
    ...
    def _FetchOne(self, opt, project):
        ...
        finish = time.time()
        del self._sync_dict[k]
        return _FetchOneResult(success, errors, project, start, finish, remote_fetched)
    ...
    def _SmartSyncSetup(self, opt, smart_sync_manifest_path, manifest):
        if not manifest.manifest_server:
            ...
            # TODO replace this with an exit error
            raise RepoExitError()
            # sys.exit(1)
    ...
    def _UpdateManifestProject(self, opt, mp, manifest_name):
        ...
        if mp.HasChanges:
            ...
            if not clean:
                # TODO - Update this with an exit error
                # sys.exit(1)
                raise RepoExitError()
    ...
    def Execute(self, opt, args):
        ...
        try:
            ...
            errors.append(e)
            raise RepoExitError(aggregate_errors=errors)
    ...
    def _ExecuteHelper(self, opt, args, errors):
        ...
        if opt.mp_update:
            try:
                ...
                # TODO handle this with an appropiate exit Error.
                raise
        else:
        ...
        for m in self.ManifestList(opt):
            ...
            if self.UpdateProjectList(opt, m):
                ...
                if opt.fail_fast:
                    ...
                    # TODO - raise an exit error - Also append to errors
                    raise RepoExitError(aggregate_errors=errors)
                    # sys.exit(1)
