...
"""
  ...
  def _GCProjects(self, projects):
    objdirs = {}
    for project in projects:
      objdirs[project.objdir] = project.bare_objdir
    ...
    if jobs < 2:
      for bare_objdir in objdir.values():
        bare_objdir.gc('--auto')
      return
    ...
    def GC(bare_objdir):
      try:
        try:
          bare_objdir.gc('--auto', config=config)
        except GitError:
          err_event.set()
        except:
          err_event.set()
    ...
    for bare_objdir in objdirs.values():
      if err_event.isSet():
        break
      sem.acquire()
      t = _threading.Thread(target=GC, args=(bare_objdir,))
      t.daemon = True
      threads.add(t)
      t.start()
