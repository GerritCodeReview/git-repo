...
"""
  ...
  def _GCProjects(self, projects, opt, err_event):
    ...
    for project in projects:
      ...
      if not project.use_git_worktrees:
        if len(project.manifest.GetProjectsWithName(project.name, all_manifests=True)) > 1:
        ...
        elif (project.config.GetBoolean('extensions.preciousObjects') and
              not project.config.GetBoolean('repo.preservePreciousObjects')):
          # We (probably) should not have preciousObjects=True.  Fix that.
          # The user can override this by setting repo.preservePreciousObjects.
          project.config.SetBoolean('extensions.preciousObjects', None)
