...
"""
    ...
    def _FetchOne(self, opt, sync_dict, project):
        ...
        """
        start = time.time()
        k = f"{project.name}:{start}"
        print(f"adding proj {k}")
        sync_dict[k] = start
        success = False
        ...
        try:
            ...
            print(f"raise deleting proj {k}")
            del sync_dict[k]
            raise

        finish = time.time()
        print(f"deleting proj {k}")
        del sync_dict[k]
    ...
    def _Fetch(self, projects, opt, err_event, ssh_proxy):
        ...
        sync_dict = multiprocessing.Manager().dict()
        ...
        def _MonitorSyncLoop():
            while True:
                if sync_event.is_set():
                    return
                msg = self._GetLongestSyncMessage(sync_dict)
                if msg:
                    pm.update(inc=0, msg=msg)
                else:
                    pm.refresh()
                time.sleep(1)
