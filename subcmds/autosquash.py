...
class Autosquasher:
    ...
    def ListCommitsWithBranches(self):
        # git log --expand-tabs --format=format:"%h %D %gd"
      command = GitCommand(self.project, ['log', '-n', '100','--expand-tabs', r'--format=format:"%h %D %gd"'], capture_stdout=True, capture_stderr=True)
      if command.Wait() != 0:
...
class Autosquash(Command):
  COMMON = True
  helpSummary = "Squashes all your commits in your current branch down to the merge branch so they can be uploaded"
  helpUsage = """
...
"""
  ...
  def Execute(self, opt, args):
    ...
    for p in projects:
      print('project: %s project.CurrentBranch: %s project.GetBranch(cb): %s' % (p, p.CurrentBranch, p.GetBranch(p.CurrentBranch)))
    ...
    try:
      if (len(projects) == 1):
        ...
        print('WARNING: Autosquash aborted. Autosquash doesn\'t work on multiple projects. Did you make sure to specify only the current directory?')
