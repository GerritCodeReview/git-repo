    ...
    def GetProjectPaths(self, name, path, remote):
        ...
        if self.IsMirror:
            ...
            if os.path.exists(gitdir) or not self.UseGitWorktrees:
                # If {gitdir}/objects is a directory, then use that rather than
                # {repodir}/project-objects. (That is listed as an alternate.)
                objdir = gitdir
                if not platform_utils.isdir(os.path.join(gitdir, 'objects')):
