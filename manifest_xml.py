...
"""
    ...
    def _ParseManifest(self, node_list):
        ...
        for node in itertools.chain(*node_list):
            ...
            if node.nodeName == "extend-project":
                ...
                for p in self._projects[name]:
                    ...
                    if revision:
                        if base:
                            if p.revisionExpr and p.revisionExpr != base:
                                failed_revision_changes.append(
                                    "extend-project name %s mismatch base %s vs revision %s"
                                    % (name, base, p.revisionExpr)
                                )
                    ...
                    for p in projects:
                        if name == projname and not path:
                            if base:
                                if p.revisionExpr and p.revisionExpr != base:
                                    failed_revision_changes.append(
                                        "remove-project name %s mismatch base %s vs revision %s"
                                        % (name, base, p.revisionExpr)
                                    )
                            del self._paths[p.relpath]
                            ...
                            if base:
                                if p.revisionExpr and p.revisionExpr != base:
                                    failed_revision_changes.append(
                                        "remove-project path %s mismatch base %s vs revision %s"
                                        % (p.relpath, base, p.revisionExpr)
                                    )
        ...
        if failed_revision_changes:
            raise ManifestParseError(
                "revision base check failed, rebase patches and update base hashes for: ",
                failed_revision_changes,
            )
