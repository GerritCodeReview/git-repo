...
class _XmlSubmanifest:
    ...
    Attributes:
      ...
      local_manifest: a string, a path relative to the parent manifest's directory
          that will be copied into the local_manifest directory of the submanifest.
    ...
    def __init__(
        ...
        if self.local_manifest_src:
            ...
            # local_manifest_dest_dir = os.path.join(parent.path_prefix, self.relpath, LOCAL_MANIFESTS_DIR_NAME)
            local_manifest_dest_dir = os.path.join(self.parent.SubmanifestInfoDir(self.parent.path_prefix, SUBMANIFEST_DIR), self.relpath, LOCAL_MANIFESTS_DIR_NAME)
            # self._ValidateFilePaths("copyfile", local_manifest_src, local_manifest_dest)
            self.AddCopyFile(self.local_manifest_src, local_manifest_dest, local_manifest_dest_dir)
    ...
    def AddCopyFile(self, src, dest, topdir):
        ...
        """
        print("AddCopyFile: %s, %s, %s, %s" % (self.parent.SubmanifestInfoDir(self.parent.path_prefix, "manifests"), src, topdir, dest))
        self.copyfiles.append(_CopyFile(self.parent.SubmanifestInfoDir(self.parent.path_prefix, "manifests"), src, topdir, dest))
...
"""
    ...
    def _ParseSubmanifest(self, node):
        ...
        if local_manifest == "":
            ...
            if msg:
                raise ManifestInvalidPathError(
                    f'<submanifest> invalid "local-manifest": {local_manifest}: {msg}'
                )
