...
class _XmlRemote(object):
  ...
  def ToRemoteSpec(self, projectName, namespace=None):
    ...
    # Check if parent has a remote with the same name.
    if namespace and namespace._parent_has_remote(self.name):
      if self.remoteAlias is None:
        self.remoteAlias = self.name
...
class XmlManifest(object):
  ...
  def Save(self, fd, peg_rev=False, peg_rev_upstream=True, groups=None):
    ...
    def output_project(parent, parent_node, p):
      ...
      if not (d.remote or
              p.remote.orig_name != remoteName or
              p.remote.name != remoteName):
        e.setAttribute('remote', p.remote.orig_name)
      if peg_rev:
        if self.IsMirror:
      ...
      if node.nodeName == 'remote':
        ...
        if added is True:
          if ns._parent is None:
            ...
            # Check if parent has a remote with the same name.
            if ns._parent_has_remote(remote.name):
  ...
  def _ParseProject(self, node, namespace, parent=None, **extra_proj_attrs):
    ...
    _, default_node = namespace._get_default()
    remote_name = node.getAttribute('remote')
    if not remote_name:
      remote_name = default_node.remote.name
    if not remote_name:
    ...
    if not revisionExpr:
      revisionExpr = default_node.revisionExpr
    if not revisionExpr:
    ...
    sync_s = node.getAttribute('sync-s')
    if not sync_s:
      sync_s = default_node.sync_s
    else:
      sync_s = sync_s.lower() in ("yes", "true", "1")

    sync_tags = node.getAttribute('sync-tags')
    if not sync_tags:
      sync_tags = default_node.sync_tags
    else:
    ...
    dest_branch = node.getAttribute('dest-branch') or default_node.destBranchExpr

    upstream = node.getAttribute('upstream') or default_node.upstreamExpr
    ...
    project = Project(namespace_name = namespace._name,
                      ...
                      name = name,
                      remote = remote.ToRemoteSpec(name,
                        self._namespaces[remote_namespace]),
                      gitdir = gitdir,
                      objdir = objdir,
                      worktree = worktree,
                      relpath = relpath,
    ...
    for n in node.childNodes:
      ...
      if n.nodeName == 'project':
        project.subprojects.append(self._ParseProject(
          n, namespace._name, parent = project))
...
class _ManifestNamespace(object):
  ...
  def _parent_has_remote(self, name):
      if self._parent is not None:
        _, v = self._parent._get_remote(name)
        if v:
          return True
      return False
  ...
  def _add_default(self, default):
    if default is None:
      return
    ns, default_node = self._get_default()
    if default_node is None:
      self._default = default
      return
    if default_node != default:
      if self._name != ns:
        self._default = default
      else:
  ...
  def _get_default(self):
    ...
    if not default_node:
      if self._parent is not None:
          ns_name, default_node = self._parent._get_default()
    return ns_name, default_node
...
class GitcManifest(XmlManifest):
  ...
  def _ParseProject(self, node, namespace, parent=None):
    ...
    return super(GitcManifest, self)._ParseProject(node, namespace, parent=parent,
