...
class _XmlRemote(object):
  ...
  def ToRemoteSpec(self, projectName, namespace=None):
    ...
    if namespace and namespace._parent:
      # check if parent has a remote with same name
      _, v = namespace._parent._get_remote(self.name)
      if v:
      ...
      if node.nodeName == 'remote':
        ...
        if added is True:
          if ns._parent is None:
            ...
            # check if parent has a remote with same name
            _, v = ns._parent._get_remote(remote.name)
            if v:
