...
# Add all projects from local manifest into a no superproject group.
LOCAL_NO_SUPERPROJECT_GROUP = 'local-no-superproject-group'
        ...
        try:
          for local_file in sorted(platform_utils.listdir(self.local_manifests)):
            if local_file.endswith('.xml'):
              ...
              nodes.append(self._ParseManifestXml(
                  local, self.repodir, parent_groups=LOCAL_NO_SUPERPROJECT_GROUP,
                  restrict_includes=False))
        except OSError:
          pass
