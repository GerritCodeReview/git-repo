...
def readlink(path):
    ...
    if reparse_point_handle == INVALID_HANDLE_VALUE:
        _raise_winerror(
            get_last_error(), 'Error opening symbolic link \"%s\"'.format(path)
        )
    ...
    if not io_result:
        _raise_winerror(
            get_last_error(), 'Error reading symbolic link \"%s\"'.format(path)
        )
    ...
    _raise_winerror(
        ERROR_NOT_SUPPORTED, 'Error reading symbolic link \"%s\"'.format(path)
    )
