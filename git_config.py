...
class SyncState(object):
  ...
  def __init__(self, config, options, superproject):
    ...
    Saves |options|, superproject and repo.*, branch.* and remote.*
    parameters from |config| object. It also saves current time as
    synctime.
    ...
    Args:
      ...
      superproject: Sync is using superproject or not.
    """
    self._config = config
    now = datetime.utcnow()
    self._Set('synctime', now.strftime('%d/%m/%Y %H:%M:%S'))
    self._Set('version', '1.0')
    self._Set('superproject', superproject)
