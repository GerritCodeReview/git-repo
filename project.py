    ...
    if is_sha1 and current_branch_only:
      ...
      if not self._CheckForSha1():
        if current_branch_only and depth:
          # Try to sync current branch only with depth set to None
          return self._RemoteFetch(name=name,
                                   current_branch_only=current_branch_only,
                                   initial=False, quiet=quiet, alt_dir=alt_dir, depth=None)
        else:
          # Avoid infinite recursion: try to sync all branches with depth set to None
          return self._RemoteFetch(name=name,
                                   current_branch_only=False,
                                   initial=False, quiet=quiet, alt_dir=alt_dir, depth=None)
