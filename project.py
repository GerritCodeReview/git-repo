...
def link_file(ln_src, absDest):
  ...
  if not platform_utils.islink(absDest) or (platform_utils.readlink(absDest) != ln_src):
    try:
      # remove existing file first, since it might be read-only
...
class _LinkFile(object):
  ...
  def _Link(self):
    ...
    if os.path.exists(absSrc):
      ...
      ln_src = absSrc if self.is_abs_src else os.path.relpath(absSrc, os.path.dirname(self.abs_dest))
      ...
      if os.path.exists(absDestDir) and not platform_utils.isdir(absDestDir):
        ...
        for absSrcFile in absSrcFiles:
          ...
          ln_src_dir = absSrcDir if self.is_abs_src else os.path.relpath(absSrcDir, absDestDir)
