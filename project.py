...
class Project(object):
    ...
    def __init__(
        ...
        Args:
          ...
          optimized_fetch: If True, when a project is set to a sha1 revision,
                           only fetch from the remote if the sha1 is not present
                           locally.
    ...
    def DeleteWorktree(self, quiet=False, force=False):
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                ...
                print(
                    'error: %s: Failed to delete obsolete checkout; remove manually, '
    ...
    def _RemoteFetch(
        ...
        if current_branch_only:
            ...
            if is_sha1 or tag_name is not None:
                if self._CheckForImmutableRevision():
                    if verbose:
                        print(
                            'Skipped fetching project %s (already have persistent ref)'
                            % self.name
                        )
                    return True
        ...
        if not current_branch_only:
            # Fetch whole repo
            spec.append(
                str((u'+refs/heads/*:') + remote.ToLocal('refs/heads/*'))
            )
    ...
    def Sync(
        ...
        if git_superproject.UseSuperproject(use_superproject, self.manifest):
            ...
            if not sync_result.success:
                ...
                print(
                    f'warning: git update of superproject {submanifest}failed, repo '
