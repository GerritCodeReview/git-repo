...
def _FirstLines(lines, limit=10):
...
class Project:
    ...
    def Sync_LocalHalf(
        ...
        if not isinstance(self, MetaProject):
            ...
            if mp.reproject_cmd and not mp.use_local_gitdirs:
                fail(
                    LocalSyncFail(
                        "repo.reprojectcmd requires repo.uselocalgitdirs to be enabled",
                        project=self.name,
                    )
                )
                return
    ...
    def _ReprojectCheckout(self, revid, head, verbose=False):
    ...
    def _ReprojectBranch(self, revid, head, message, verbose=False):
    ...
    def _Reproject(self, revid, verbose=False):
