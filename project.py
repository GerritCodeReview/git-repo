...
class Project:
    ...
    def UploadForReview(
        ...
        if patchset_description:
            opts += ['m=%s' % self._PercentEncodeForGitRef(patchset_description)]
        if opts:
    ...
    def _PercentEncodeForGitRef(self, original):
        """Applies percent-encoding for strings sent via git ref metadata.
        ...
        encoded = ''.join(c if c in safe else '%%%02X' % ord(c)
                            for c in original)

        # Spaces are not allowed in git refs; gerrit will interpret either '_'
        # or '+' (or '%20') as space. Use '_' since that has been supported the
        # longest.
