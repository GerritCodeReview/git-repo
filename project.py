    ...
    for part in components:
        if part in {".", ".."}:
            raise ManifestInvalidPathError(
                '{}: "{}" not allowed in paths'.format(subpath, part)
            )

        path = os.path.join(path, part)
        if platform_utils.islink(path):
            raise ManifestInvalidPathError(
                "{}: traversing symlinks not allow".format(path)
            )

        if os.path.exists(path):
            if not os.path.isfile(path) and not platform_utils.isdir(path):
                raise ManifestInvalidPathError(
                    "{}: only regular files & directories allowed".format(path)
                )
...
class _CopyFile:
    ...
    def _Copy(self):
        ...
        if platform_utils.isdir(src):
            raise ManifestInvalidPathError(
                "{}: copying from directory not supported".format(self.src)
            )
        if platform_utils.isdir(dest):
            raise ManifestInvalidPathError(
                "{}: copying to directory not allowed".format(self.dest)
            )
...
class Project:
    ...
    def PrintWorkTreeStatus(self, output_redir=None, quiet=False, local=False):
        ...
        for p in sorted(set(paths)):
            ...
            if i and i.src_path:
                ...
                line = " {}{}\t{}".format(i_status, f_status, p)
    ...
    def UploadForReview(
        ...
        ref_spec = "{}:refs/for/{}".format(R_HEADS + branch.name, dest_branch)
        opts = []
        if auto_topic:
    ...
    def EnableRepositoryExtension(self, key, value="true", version=1):
        ...
        self.config.SetString("extensions.{}".format(key), value)
    ...
    def _RemoteFetch(
        ...
        for try_n in range(retry_fetches):
            ...
            if not verbose and gitcmd.stdout:
                print(
                    "\n{}:\n{}".format(self.name, gitcmd.stdout),
                    end="",
                    file=output_redir,
                )
    ...
    def _FetchBundle(self, srcUrl, tmpPath, dstPath, quiet, verbose):
        ...
        with GetUrlCookieFile(srcUrl, quiet) as (cookiefile, proxy):
            ...
            with Trace("Fetching bundle: %s", " ".join(cmd)):
                if verbose:
                    print(
                        "{}: Downloading bundle: {}".format(self.name, srcUrl)
                    )
    ...
    def _Checkout(self, rev, quiet=False):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            if self._allrefs:
                raise GitError(
                    "{} checkout {} ".format(self.name, rev), project=self.name
                )
    ...
    def _CherryPick(self, rev, ffonly=False, record_origin=False):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            if self._allrefs:
                raise GitError(
                    "{} cherry-pick {} ".format(self.name, rev),
                    project=self.name,
                )
    ...
    def _Revert(self, rev):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            if self._allrefs:
                raise GitError(
                    "{} revert {} ".format(self.name, rev), project=self.name
                )
    ...
    def _ResetHard(self, rev, quiet=True):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            raise GitError(
                "{} reset --hard {} ".format(self.name, rev), project=self.name
            )
    ...
    def _Rebase(self, upstream, onto=None):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            raise GitError(
                "{} rebase {} ".format(self.name, upstream), project=self.name
            )
    ...
    def _FastForward(self, head, ffonly=False):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            raise GitError(
                "{} merge {} ".format(self.name, head), project=self.name
            )
