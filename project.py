...
class ManifestProject(MetaProject):
  ...
  def SyncWithPossibleInit(self, submanifest, verbose=False,
    ...
    return self.Sync(
        ...
        groups=opts.manifest_groups,
        platform=opts.manifest_platform,
        mirror=opts.mirror,
  ...
  def Sync(self, _kwargs_only=(), manifest_url='', manifest_branch=None,
    ...
    default_groups_str = ','.join(
        (self.manifest.GetDefaultGroupsStr(),
         platformize(self._platform_name)))
    if platform == 'auto' and groupstr == default_groups_str:
      groupstr = None
