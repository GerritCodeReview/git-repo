...
class ReviewableBranch(object):
  ...
  def GetPublishedRefs(self):
    refs = {}
    output = self.project.bare_git.ls_remote(self.branch.remote.SshReviewUrl(self.project.UserEmail),
                                             'refs/changes/*')
...
# Direct Git Commands ##
  ...
  def _FetchBundle(self, srcUrl, tmpPath, dstPath, quiet):
    ...
    with GetUrlCookieFile(srcUrl, quiet) as (cookiefile, proxy):  # pylint: disable=unused-variable
      if cookiefile:
