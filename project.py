...
## Direct Git Commands ##
  ...
  def _InitWorkTree(self):
    if not os.path.exists(self.worktree):
      ...
      self._CopyFiles()
    elif os.path.islink(os.path.join(self.worktree, '.git', HEAD)):
      # This project has an old-style symlink to the contents of .git/.
      # Convert to the new .git file.  Copy HEAD and index, then remove
      # the .git folder and replace with a .git file. Note that we must
      # check if .git/HEAD is a symlink, not check if .git is a folder,
      # because this code is also called for .repo/repo and .repo/manifests
      # which need the .git folder but don't use symlinks.
