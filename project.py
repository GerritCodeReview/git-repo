...
def _FetchCommitMsgHook(hook_list, review):
  ...
  """
  if review and not review.startswith("http"):  # fudge as we only support http/s here
    review = None
  ...
  rdir = (review or "https://gerrit-review.googlesource.com/").replace('/', '_')
  ...
  if os.path.isfile(new_hook):
    if time.time() - os.path.getmtime(new_hook) < 3600*6:
      return hook_list
    os.remove(new_hook)
  url = urljoin(review or "https://gerrit-review.googlesource.com/",
...
## Direct Git Commands ##
  ...
  def _InitHooks(self):
    ...
    if not os.path.exists(hooks):
      os.makedirs(hooks)
    stock_hooks = _ProjectHooks()
    stock_hooks = _FetchCommitMsgHook(stock_hooks, self.remote.review)
