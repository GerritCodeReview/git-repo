...
Sync unless explicitly overridden on the command line. If the manifest server
cannot be reached or returns an error during an implicit Smart Sync, `repo sync`
logs a warning and falls back to syncing ToT. Options that specify an explicit
target or manifest source (such as `-t`/`--smart-tag` or `-m`/`--manifest-name`)
will also disable the default Smart Sync behavior.
