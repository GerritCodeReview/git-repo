...
For custom proxying or authentication, `git-repo` supports pluggable remote helpers. You can declare a helper binary via the optional `helper` attribute on the `<manifest-server>` element.

If the `helper` attribute is present in `<manifest-server>`:
1.  `git-repo` searches your system `PATH` for the specified helper binary (e.g., `repo-remote-sso`).
...
3.  The helper must output a single-line JSON object on stdout and exit:
    ...
    *   On failure, return `status: "error"` and a detailed error message:
        ...
        ```
4.  `git-repo` routes the XML-RPC request through the returned proxy address.
