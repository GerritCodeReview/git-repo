...
file named `post-sync.py` is created.  Repo will dynamically load the module
...
It is not run on network-only sync (as the network only updates the internal
cache, and does not create or update the local project checkout), and it
does not run when the checkout phase failed. The hook is called once after
all projects are checked out and a list of projects for which the checkout
completed is passed on to the hook.
