...
will never be checked out are not downloaded. Without the filter Git still
downloads every blob in the project and sparse-checkout merely declines to
write most of them to disk, so the filter is what turns sparse-checkout into a
saving in transfer size and server load. The filter from
...
Because that filter is only useful if the objects it excludes are not fetched
by some other route, a project that takes the implied `--filter=blob:none` also
skips the `/clone.bundle` download: the bundle is an unfiltered pack of every
object, so fetching it would undo the filter. Projects that set the filter
explicitly, or that are listed in `repo init --partial-clone-exclude`, keep
using the clone bundle.

Attribute `path`: A path relative to the project root to include in the
sparse-checkout. The path is validated like `<copyfile>` and `<linkfile>`
paths: it must stay inside the project, so absolute paths, `.` or `..`
components, `~`, and `.git` or `.repo*` components are all rejected. A trailing
`/` is allowed.
