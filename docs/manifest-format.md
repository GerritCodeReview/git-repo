...
`stateless`, repo will attempt to keep the local repository as small as
possible by performing an in-situ shallow re-fetch and garbage collection
instead of an incremental fetch.  This is useful for projects that contain
...
2. The working tree is clean (no uncommitted changes, no untracked files,
   no local commits not present in the manifest revision).
