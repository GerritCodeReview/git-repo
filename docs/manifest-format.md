...
meaning one of them is required. If only name is specified, all mounted
projects with that name are removed.

If both name and path are specified, only projects with same name and
path are removed, meaning same name in other locations are kept.

If only path is specified, a matching name for that is found and
