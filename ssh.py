...
class ProxyManager:
    ...
    def _open_unlocked(self, host, port=None):
        ...
        # Git protocol V2 is a new feature in git 2.18.0, made default in git 2.26.0
        # It is faster and more efficient than V1.
        # To enable it when using SSH, the environment variable GIT_PROTOCOL must be set
        # in the SSH side channel when establishing the connection to the git server.
        ...
        # is established manually over ControlMaster via the repo-tool, it must be passed in
        # explicitly instead.
        # Based on https://git-scm.com/docs/gitprotocol-pack#_extra_parameters, GIT_PROTOCOL
        # is considered an "Extra Parameter" and must be ignored by servers that do not
        # understand it. This means that it is safe to set it even when connecting to older servers.
        # It should also be safe to set the environment variable for older local git versions, since it is only part
        # of the ssh side channel.
...
def _get_git_protocol_version() -> str:
    """
    Get git protocol version based on git config, with fallback to deducing the default
    value based on the git version.
