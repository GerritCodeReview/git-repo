...
*   [`agent_configs.textproto`](agent_configs.textproto): Defines the active AI
...
The agents defined in `agent_configs.textproto` are configured to run
...
2.  **Add new skills**:
    ...
    *   Add a `SKILL.md` following the established format (see existing skills
        for reference).
    *   Register the new skill in `agent_configs.textproto` by adding it to the
