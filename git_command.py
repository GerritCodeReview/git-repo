...
class GitCommand(object):
  def __init__(self,
    ...
    if disable_editor:
      env['GIT_EDITOR'] = ':'
    if ssh_proxy:
      platform_utils.putenv('REPO_SSH_SOCK', ssh_sock(), env)
      platform_utils.putenv('GIT_SSH', _ssh_proxy(), env)
    ...
    if 'http_proxy' in env and 'darwin' == sys.platform:
      ...
      if p is not None:
        s = p + ' ' + s
      platform_utils.putenv('GIT_CONFIG_PARAMETERS', s)
    if 'GIT_ALLOW_PROTOCOL' not in env:
      env['GIT_ALLOW_PROTOCOL'] = (
          'file:git:http:https:ssh:persistent-http:persistent-https:sso:rpc')
    platform_utils.putenv('GIT_HTTP_USER_AGENT', user_agent.git, env)
    ...
    if bare:
      if gitdir:
        platform_utils.putenv(GIT_DIR, gitdir, env)
      cwd = None
