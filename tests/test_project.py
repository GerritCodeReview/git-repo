...
class SyncOptimizationTests(unittest.TestCase):
    ...
    def test_check_immutable_revision_plain_project_upstream_ancestor(self):
        """Non-shallow projects verify upstream ancestry for immutable revisions."""
        with utils_for_test.TempGitTree() as tempdir:
            proj = _create_mock_project(tempdir)
            proj.bare_git = project.Project._GitGetByExec(
                proj, bare=True, gitdir=proj.gitdir
            ...
            # 1. When revision is commit1 and upstream tracking ref is at commit2:
            # commit1 is an ancestor of refs/remotes/origin/main -> True
            ...
            # 2. When revision is commit2 and upstream tracking ref is at commit1 (behind):
            # commit2 is NOT an ancestor of refs/remotes/origin/main -> False
            ...
            # commit2 exists in ODB, so shallow returns True even if upstream is behind.
            self.assertTrue(
                proj._CheckForImmutableRevision(use_superproject=False, depth=1)
            )

            # 4. If gitdir has shallow file, shallow check also skips upstream ancestry.
            with open(os.path.join(proj.gitdir, "shallow"), "w") as f:
                f.write("")
            self.assertTrue(
                proj._CheckForImmutableRevision(use_superproject=False)
    ...
    def test_sync_network_half_stale_upstream_fetches(self):
