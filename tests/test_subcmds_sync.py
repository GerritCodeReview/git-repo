...
class TestSmartSyncSetupRemoteHelper(unittest.TestCase):
    ...
    def test_smart_sync_setup_with_helper(
        ...
        import subprocess
        self.manifest.manifest_server = "sso://android-smartsync.corp.google.com/manifestserver"
        mock_which.return_value = "/fake/bin/repo-remote-sso"

        # Mock subprocess to return a JSON with status ok and proxy address
        mock_process = mock.MagicMock()
        mock_process.stdout.readline.return_value = '{"status":"ok","message":"127.0.0.1:999"}\n'
        mock_popen.return_value = mock_process

        # Mock XML-RPC server call
        mock_server = mock.MagicMock()
        mock_server.GetApprovedManifest.return_value = [True, "<manifest></manifest>"]
        ...
        # Verify transport was created with the proxy returned by helper (with http:// prepended)
        mock_transport_class.assert_called_once_with(
            self.manifest.manifest_server, proxy="http://127.0.0.1:999"
        )
        
        # Verify Server was created with rewritten HTTP URL
    ...
    def test_smart_sync_setup_helper_error(self, mock_popen, mock_which):
        ...
        mock_process = mock.MagicMock()
        mock_process.stdout.readline.return_value = '{"status":"error","message":"uplink-helper failed"}\n'
        mock_popen.return_value = mock_process
    ...
    @mock.patch("shutil.which")
    def test_smart_sync_setup_missing_helper_custom_scheme(self, mock_which):
        """Test _SmartSyncSetup when helper is missing for custom scheme."""
        self.manifest.manifest_server = "sso://android-smartsync.corp.google.com/manifestserver"
        ...
        self.assertIn(
            "unsupported manifest server scheme 'sso'",
            str(context.exception),
        )
        self.assertIn(
            "please ensure the local helper 'repo-remote-sso' is installed",
            str(context.exception),
        )

    def test_get_branch_fallback(self):
        """Test _GetBranch fallback when branch has no merge config."""
        manifest_project = mock.MagicMock()
        manifest_project.CurrentBranch = "local-branch"
        
        # Mock GetBranch to return a branch config with no merge
        mock_branch = mock.MagicMock()
        mock_branch.merge = None
        manifest_project.GetBranch.return_value = mock_branch
        
        # Mock revisionExpr
        manifest_project.revisionExpr = "refs/heads/main"
        
        branch = self.cmd._GetBranch(manifest_project)
        self.assertEqual(branch, "main")
        manifest_project.GetBranch.assert_called_once_with("local-branch")
