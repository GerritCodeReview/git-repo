...
class SyncTimesTest(unittest.TestCase):
    ...
    def test_p90_insufficient_samples(self):
...
class SyncCheckLatencyTest(unittest.TestCase):
    ...
    def test_warn_when_failed_and_above_p90(self, mock_warn, _mock_target):
    ...
    @mock.patch.object(sync.logger, "warning")
    def test_no_warn_when_failed_and_below_p90(self, mock_warn):
        self.cmd._sync_times.GetP90.return_value = 100.0
        self.cmd._CheckLatencyAndWarn(self.opt, 80.0, success=False)
        mock_warn.assert_not_called()

    @mock.patch.object(sync.logger, "warning")
    def test_no_warn_when_below_p90(self, mock_warn):
        self.cmd._sync_times.GetP90.return_value = 100.0
    ...
    @mock.patch.object(sync.logger, "warning")
    def test_warn_when_above_p90_with_trace2_log(self, mock_warn, _mock_target):
        self.cmd._sync_times.GetP90.return_value = 100.0
        self.cmd.git_event_log.full_sid = "sid-123"
    ...
    @mock.patch.object(sync.logger, "warning")
    def test_no_warn_or_record_on_repo_changed_exception(self, mock_warn):
        cmd = sync.Sync()
