...
class TestRepoWorkspace:
    """Shared test helper for constructing a `.repo` test workspace."""
...
""",
    ...
    def xml_manifest(self) -> manifest_xml.XmlManifest:
        """Parse and return the manifest from this workspace."""
        return manifest_xml.XmlManifest(
            str(self.repodir), str(self.manifest_file)
        )

    def create_manifest_project(
        self,
        *,
        name: str,
        path: str,
    ...
    def create_manifest_projects(
        self, projects: List[Tuple[str, str]]
    ) -> List[Path]:
        """Bulk-create multiple manifest projects."""
        return [
            self.create_manifest_project(name=name, path=path)
