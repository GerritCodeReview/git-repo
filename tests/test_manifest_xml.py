...
""")
    ...
    for this_proj in include_m.projects:
      if this_proj.name == 'root-name1':
        # Check include group not set on root level proj
        self.assertNotIn('level1-group', this_proj.groups)
      if this_proj.name == 'root-name2':
        # Check root proj group not removed
        self.assertIn('r2g1', this_proj.groups)
      if this_proj.name == 'level1-name1':
        # Check level1 proj has inherited group level 1
        self.assertIn('level1-group', this_proj.groups)
      if this_proj.name == 'level2-name1':
        # Check level2 proj has inherited group levels 1 and 2
        self.assertIn('level1-group', this_proj.groups)
        self.assertIn('level2-group', this_proj.groups)
        # Check level2 proj group not removed
        self.assertIn('l2g1', this_proj.groups)
