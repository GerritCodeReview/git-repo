...
class ManifestValidateFilePaths(unittest.TestCase):
  ...
  def testNormalPath(self):
    ...
    self.checkBoth('foo', 'bar')
    self.checkBoth('foo/bar', 'bar')
    self.checkBoth('foo', 'bar/bar')
    self.checkBoth('foo/bar', 'bar/bar')

  def testSymlinkTargets(self):
  ...
  def testBadPaths(self):
    ...
    for path in PATHS:
      self.assertRaises(
          error.ManifestInvalidPathError, self.checkBoth, path, 'a')
      self.assertRaises(
          error.ManifestInvalidPathError, self.checkBoth, 'a', path)
