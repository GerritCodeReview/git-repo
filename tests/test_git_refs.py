...
SUPPORTS_REFTABLE = _supports_reftable()
SUPPORTS_REFS_MIGRATE = _supports_refs_migrate()
...
class GitRefsTest(unittest.TestCase):
    ...
    def test_reads_files_refs(self):
    ...
    def test_reads_reftable_refs(self):
        repo = self._init_repo(reftable=True)
        gitdir = os.path.join(repo, ".git")
        refs = git_refs.GitRefs(gitdir)
    ...
    def test_updates_when_refs_change_files(self):
        repo = self._init_repo(reftable=False)
        gitdir = os.path.join(repo, ".git")
        refs = git_refs.GitRefs(gitdir)
    ...
    def test_updates_when_refs_change_reftable(self):
        repo = self._init_repo(reftable=True)
        gitdir = os.path.join(repo, ".git")
        refs = git_refs.GitRefs(gitdir)
