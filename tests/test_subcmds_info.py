...
class InfoCommand(unittest.TestCase):
    ...
    def test_summary_option_parses(self):
        """The -s/--summary flag should set opt.summary."""
        for argv in (["-s"], ["--summary"]):
            opts, _ = self._parse(argv)
            self.assertTrue(opts.summary)

    def test_summary_default_false(self):
        """opt.summary should default to False."""
        opts, _ = self._parse([])
        self.assertFalse(opts.summary)

    def test_summary_skips_projects(self):
        """--summary should print manifest info and return without iterating
        projects."""
        opts, args = self._parse(["--summary"])
    ...
    def test_no_summary_calls_diff_info(self):
        """Without --summary or --overview, _printDiffInfo should be called."""
    ...
    def test_summary_with_overview(self):
        """--summary should take priority over --overview."""
        opts, args = self._parse(["--summary", "--overview"])
