...
class TestRepoMon(unittest.TestCase):
    ...
    def test_maybe_report_repomon_below_threshold(self):
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(
                self.cmd, "_IsRepoMonInstalled", return_value=False
            ):
                with mock.patch("builtins.print") as mock_print:
                    self.cmd._MaybeReportRepoMon(self.opt, 800)
                    mock_print.assert_not_called()
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(
                ...
                with mock.patch("builtins.print") as mock_print:
                    ...
                    self.assertIn(
                        "Sync took over 900 seconds", mock_print.call_args[0][0]
                    )
                    self.assertIn(
    ...
    def test_maybe_report_repomon_initial_sync(self):
        """Test no message is printed for initial sync."""
        self.cmd._new_project_count = 1
        with mock.patch.object(
            self.cmd, "_IsGoogleplexAndroid", return_value=True
        ):
            with mock.patch.object(
                ...
                with mock.patch("builtins.print") as mock_print:
                    self.cmd._MaybeReportRepoMon(self.opt, 1000)
                    mock_print.assert_not_called()
                    self.cmd.git_event_log.LogDataConfigEvents.assert_not_called()

    def test_maybe_report_repomon_already_installed(self):
        """Test no message is printed if repomon is installed."""
        with mock.patch.object(
            self.cmd, "_IsGoogleplexAndroid", return_value=True
        ):
            with mock.patch.object(
                self.cmd, "_IsRepoMonInstalled", return_value=True
            ):
                with mock.patch("builtins.print") as mock_print:
                    self.cmd._MaybeReportRepoMon(self.opt, 1000)
                    mock_print.assert_not_called()
