...
class AllCommands(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.repodir = self.tempdir.joinpath(".repo")
        self.manifest_dir = self.repodir.joinpath("manifests")
        self.manifest_file = str(
            self.repodir.joinpath(manifest_xml.MANIFEST_FILE_NAME)
        )
        self.local_manifest_dir = self.repodir.joinpath(
    ...
    def initTempGitTree(self, git_dir):
        ...
        if git_command.git_require((2, 28, 0)):
            ...
            templatedir = Path.joinpath(self.tempdirobj.name, ".test-template")
            os.makedirs(templatedir)
            with open(templatedir.joinpath("HEAD"), "w") as fp:
    ...
    def getXmlManifestWith8Projects(self):
        ...
        gitdir = self.repodir.joinpath("manifests.git")
        os.mkdir(gitdir)
        with open(gitdir.joinpath("config"), "w") as fp:
            fp.write(
                """[remote "origin"]
        ...
        for x in range(1, 9):
            os.makedirs(self.repodir.joinpath(f"projects/tests/path{x}.git"))
            os.makedirs(
                self.repodir.joinpath(f"project-objects/project{x}.git")
            )
            git_path = self.tempdir.joinpath(f"tests/path{x}")
