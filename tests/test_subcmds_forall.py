...
class AllCommands(unittest.TestCase):
    ...
    def getXmlManifestWith8Projects(self, data):
        ...
        # Set up empty projects to match the test manifest
        for x in range(1, 9):
            os.makedirs(
                os.path.join(
        ...
        # Set up git repo for parsing to work
        ...
        # Initialize the manifest itself
    ...
    def test_forall_all_projects_get_called_once(self, mock_stdout):
        """Test that all projects get a command run once each (and only once)
        instead of for example returning after first run."""
        ...
        # Echo the project name for all manifest projects
        ...
        with mock.patch.object(
            ...
            # Verify that we didnt get more lines than expected
            assert line_count == 8
