...
from subcmds.wipe import WipeError
...
class WipeUnitTest(unittest.TestCase):
    ...
    def _create_mock_project(self, name, objdir_path=None, has_changes=False):
        ...
        proj.gitdir = os.path.join(
            self.tempdir, ".repo/projects", f"{name}.git"
        )
        if objdir_path:
            proj.objdir = objdir_path
        else:
            proj.objdir = os.path.join(
                self.tempdir, ".repo/project-objects", f"{name}.git"
            )
        ...
        def side_effect_delete_worktree(force=False):
        ...
        proj.DeleteWorktree.side_effect = side_effect_delete_worktree
    ...
    def test_wipe_shared_project_no_force_raises_error(self):
        ...
        with self.assertRaises(WipeError) as e:
